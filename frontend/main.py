...
async def chat(req: Request):
    ...
    async with httpx.AsyncClient(headers=_auth_headers(), timeout=120) as client:
        ...
        # Fallback: inspect history & task artifacts for complete agent messages
        if last_task is not None:
            for hist_msg in getattr(last_task, "history", []) or []:
                if getattr(hist_msg, "role", None) == Role.agent:
                    extracted = _extract_parts(getattr(hist_msg, "parts", []))
                    for p in extracted:
                        if p not in parts and "Cannot add session to memory" not in p.get("text", ""):
                            parts.append(p)

            if not got_artifact_update:
